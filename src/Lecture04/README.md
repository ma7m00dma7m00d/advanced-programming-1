...
3. **Test Custom Exception:** Use a `try-catch` block to attempt to `Withdraw` an amount _larger_ than the balance. In the `catch` block for `InsufficientFundsException`, print the exception's message to the console.

4. **Test Validation Exception:** Use a `try-catch` block to attempt to `Deposit` a _negative_ amount. In the `catch` block for `ArgumentException`, print that validation error.
